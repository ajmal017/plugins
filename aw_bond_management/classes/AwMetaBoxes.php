...
class AwMetaBoxes {
    ...
    public static function auctionWinnerMetaBox() {
       /* error_reporting(E_ALL);
        ...
        add_meta_box(
            'AwMetaBoxesBonds', __( 'Bonds', 'AwBondManagement' ),
        ...
        add_meta_box(
            'AwMetaBoxesBids',
            esc_html__( 'Bids   ', 'AwBondManagement' ),
        ...
        add_meta_box(
            'AwMetaBoxesWinner',
            esc_html__( 'Winners', 'AwBondManagement' ),
        ...
        add_meta_box(
            'AwMetaBoxesBWICs',
            esc_html__( 'Final Results', 'AwBondManagement' ),
    ...
    public function auctionWinnerMetaBoxWinner( $auction ) {
        if($auction->status !== 'completed' ){
            echo '<div style="text-align: center;">
                    <h1 style="color: #FF0000;">BWIC not completed yet.</h1>
                </div>';
            return;
        }
        $bonds = get_post_meta($auction->ID, '_auction_meta_field_value', true);
        foreach ($bonds as $bond) {
            if( AwMetaBoxes::hasWinner($auction->ID, $bond )){
    ...
    public static function getClosedBWICsBond($auction, $bond){
        ...
        foreach ($placedBids as $key => $bid ) {
            ...
            if ($winner_id == $key) {
                $results = '<p style="background: #00FF00; font-weight: bold; color: #FFF; padding: 4px">Winner</p>';
            }else{
                $results = '<p style="background: #FF0000; font-weight: bold; color: #FFF; padding: 4px">Loser</p>';
            }
    ...
    public static function hasWinner($auction, $bond){
        $meta_key       =   'user_winning_bid_'.$auction.'_'.$bond;
        $placedBids     =   get_post_meta($auction,$meta_key,true);
