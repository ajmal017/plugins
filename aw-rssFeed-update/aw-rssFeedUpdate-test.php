...
file_put_contents(dirname(__FILE__)."/".$log_filname, print_r($logTime, true),FILE_APPEND );
file_put_contents(dirname(__FILE__)."/".$log_filname, print_r("Blogs Updated \n", true),FILE_APPEND );
		...
		foreach( $contents as $value ){
			...
			foreach( $contentssub as $itemValue ){
				...
				if( $itemValue != "" ){
					...
					// file_put_contents(dirname(__FILE__)."/AllFiles.log", print_r($itemValueSub, true),FILE_APPEND );
...
*/
		...
		if( !empty( $rsBlog ) ){
			...
			foreach ($rsBlog as $blog ) {
				...
				// if( $blogName == 'astronomi'){
				// 	$rssFeed 		= "http://iris.scanmine.com/go/pub/astronomi/xml_rss/astronomi_no.rss";
					
				// 	//** Check the Destination blog destination **//
...
function connectToDatabase( $destinationBlog ){
    ...
    if (mysqli_connect_errno()){
        ...
        return false;
  		...
  		//   	$sql  	= 	"SELECT * FROM wp_links WHERE link_visible ='Y' AND link_rss='$rssfeed'";
		// $result = $connection->query( $sql );
		// if( $result->num_rows > 0 ) {
		// 	$row = mysqli_fetch_assoc($result);			
		// 	readRSS( $connection, $row['link_url'] );
		// }		
		return $connection;
    }
