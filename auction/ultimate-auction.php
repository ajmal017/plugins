<?php
/*
  Plugin Name: Auction Plugin
  Plugin URI: 
  Description: Plugin to create auctions
...
function wdm_create_bidders_table()
{
  ...
  /* $alt_sql = "ALTER TABLE $data_table MODIFY bid DECIMAL(10,2);";
   ...
   $indx = $wpdb->get_results("SHOW indexes FROM $data_table WHERE Column_name = 'bid';");
    ...
    $alt_sql = "ALTER TABLE $data_table ADD INDEX bid(bid);";
    $wpdb->query($alt_sql);

    //add index to auction_id column as well
    $alt_sql = "ALTER TABLE $data_table ADD INDEX aid(auction_id);";
    $wpdb->query($alt_sql);
}
