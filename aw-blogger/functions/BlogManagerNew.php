...
class BlogManager {
	...
	public function createNewFeedSiteFromConfig($url) {
        ...
        for($i=0;$i<count($p1['page']);$i++){
            ...
            if(!empty($p4)){
            	$term_title = sanitize_title($p2['title']);
	            $AllPages[$term_title] = $p4;
            } else {
            	$term_title = sanitize_title($p2['title']);
    	        $AllPages[$term_title] = $p2['title'];
            }
        }
		$pages 		= serialize($AllPages);

		//file_put_contents(dirname(__FILE__)."/page_logs.log", print_r($pages, true) );
		...
		if ( $reader->hasProperty('description') )
	        ...
	        foreach ($AllPages as $slug => $name) {

				$insert_term    = "INSERT INTO `wp_terms`(`name`, `slug`) VALUES ( '$name','$slug' )";
				$conn->query($insert_term);
				$last_insert_id = $conn->insert_id;

				$wp_term_taxonomy = "INSERT INTO `wp_term_taxonomy`(`term_id`, `taxonomy`, `count`) VALUES ($last_insert_id, 'category', 0)";
				$conn->query($wp_term_taxonomy);

				file_put_contents(dirname(__FILE__)."/term_log.log", print_r($name, true),FILE_APPEND );
				file_put_contents(dirname(__FILE__)."/term_log.log", print_r("\n\n", true),FILE_APPEND );
			}
