	...
	if (file_exists( $wordpress_folder ) &&  is_dir( $wordpress_folder ) ){
		...
		for( $i = 0; $i < $count; $i++ ){
			...
			if( !empty( $errorMessages ) ){
				...
				$site_info['template'] 		= (string) $reader->getProperty('template');			//$manager->createNewFeedSiteFromConfig($newurl);
				...
				if( $errorFlag == true ){
					...
					// echo "<p style='text-align: center; font-size: 18px;'><a href='$url1'>Go Back</a></p>";
					echo "<hr>";
					continue;
				}
				//** Block Ends Here. **//
				...
				if ( !file_exists( $destinationPath ) && !is_dir( $destinationPath ) ) {
					...
					if( $result ){
						...
						// file_put_contents(dirname(__FILE__)."/cmd_log/updater.log", print_r($clearCommand, true),FILE_APPEND );
						...
						//** Update Destination Config File. **//
					   	...
					   	if(	 $resultDB ){
					   		//** Copy Datase **//
					   		// $sourceDB 		=	getSourceDb(  $siteTemplate );
					   		...
					   		if(	 $resultDBCopy ){
								...
								/*code to create domain*/
								$blogNames = $args['site_slug'];
								domainPointing($blogNames);
...
function UpdateConfigFile( $extractionPath, $site_slug ){
	...
	/*if(file_exists($config_file_final)){
		unlink( $config_file_final );
	}*/

	// chmod($config_file, 0775);
	// chmod($config_file_final, 0775);
	chmod($config_file, 0664);
	chmod($config_file_final, 0664);

	//** Change Permission of wp-content**//
	// chmod_r($extractionPath, 0777, 0777);
...
function updateSiteOptions( $site_slug , $adminEmail, $siteTitle, $siteDescription, $topic = '' ){
	...
	if ($conn->connect_error) {
		...
		$dom_pos = strpos($domain_url, "http://");
		if ($dom_pos === 0) {
		    $site_urls = $domain_url;
		} else {
		    $site_urls= "http://".$domain_url;
		}

		if (strrpos($site_urls, '/') == strlen($site_urls) - 1) {
		    $com_blogpath = $site_urls;
		}else{
		    $com_blogpath = $site_urls."/";
		}

		$blog_path 	=	$com_blogpath;
		...
		$insert_site_address = "INSERT INTO `wp_options` (`option_name`, `option_value`, `autoload`) VALUES ('site_address', '$site_slug', 'yes')";
		$conn->query($insert_site_address);
...
function deleteDummyPosts( $databaseName ){
	...
	if ($conn->connect_error) {
		...
		/*$sql_category = "DELETE FROM ` wp_term_taxonomy` WHERE `taxonomy` = 'category'";
...
//** functionality to domain pointing **//

function domainPointing($blogNames){
